...
void Launcher::playGame() // Launches the game with the arguments
{
	...
	QStringList fixed_params;
	QStringList custom_params;
	custom_params << m_customLaunchOptions.split(" ");
	fixed_params << QString("-novr");
	fixed_params << QString("-window");
	fixed_params << QString("-condebug");
	fixed_params << QString("+vr_enable_fake_vr 1");
	fixed_params << QString("+hlvr_main_menu_delay 999999");
	fixed_params << QString("+hlvr_main_menu_delay_with_intro 999999");
	fixed_params << QString("+hlvr_main_menu_delay_with_intro_and_saves 999999");
	QProcess * p = new QProcess();
	p->setWorkingDirectory(settings.value("installLocation").toString() + "/game/bin/win64/");
	p->setProgram(settings.value("installLocation").toString() + "/game/bin/win64/hlvr.exe");
	p->setArguments(fixed_params + custom_params);
	p->start();
